...
public:
    Wonder(std::string name,
           ...
           std::vector< std::vector< std::function< std::vector<Produce> (const Wonder& w, const Player& p) > > > getProduces)
        : sideA_{true},
          numCompleted_{0},
          powerUsed_{false},
          name_{name},
    ...
    void usePower() { powerUsed_ = true; }
    void resetPower() { powerUsed_ = false; }
    bool powerUsed() const { return powerUsed_; }
    ...
    std::vector<Produce> getProduce(const Player& p) const {
        ...
        for (size_t i=0; i<=numCompleted_; ++i) {
            std::vector<Produce> thisProduce = getProduces_[!sideA_][i](*this, p);
            produce.insert(produce.end(), thisProduce.begin(), thisProduce.end());
        }
        return produce;
    }
    std::vector<Produce> getProduceFromNext(const Player& p) const { return getProduces_[!sideA_][numCompleted_+1](*this, p); }
...
private:
    ...
    std::vector< std::vector< std::function< std::vector<Produce> (const Wonder& w, const Player& p) > > > getProduces_;
};
